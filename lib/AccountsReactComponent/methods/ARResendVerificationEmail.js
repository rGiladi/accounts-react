...
const ARResendVerificationEmail = new ValidatedMethod({
  ...
  run ({ email }) {
    if (Meteor.isServer) {
      ...
      if (!user) {
        throw new Meteor.Error('UserNotFound', AccountsReact.config.texts.errors.userNotFound || T9n.get(AccountsReact.config.translations.errors.userNotFound));
      }
      ...
      try {
        ...
        if (error.error === 'disabled') {
          ...
          throw new Meteor.Error('UserAlreadyVerified', AccountsReact.config.texts.errors.userAlreadyVerified || T9n.get(AccountsReact.config.translations.errors.userAlreadyVerified));
        }
      }
    }
  }
