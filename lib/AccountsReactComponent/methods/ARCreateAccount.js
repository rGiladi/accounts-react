...
const ARCreateAccount = new ValidatedMethod({
  ...
  validate: ({ username, email, password, ...profile }) => {
    ...
    if (AccountsReact.config.showReCaptcha && !profile.tempReCaptchaResponse) {
      throw new Meteor.Error('ReCaptchaError', AccountsReact.config.texts.errors.captchaVerification || T9n.get(AccountsReact.config.translations.errors.captchaVerification))
    }
