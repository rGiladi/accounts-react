...
class BaseForm extends Component {
  ...
  render () {
    ...
    // texts
    const title  = texts.title[currentState] || T9n.get(translations.title[currentState])
    const button = texts.button[currentState] || T9n.get(translations.button[currentState])
      ...
      <FormField onSubmit={(e) => e.preventDefault()} className={`ar-${currentState}`}>
        ...
        <div className='ar-fields'>
          {fields.map((fld, i) => {
            ...
            const { displayName, placeholder, errStr, ...f } = fld; // extract translated props from field props
            const props = {
              key: i,
              values,
              defaults,
              ...
              displayName: T9n.get(displayName),
              placeholder: T9n.get(placeholder),
              errStr: errStr ? T9n.get(errStr) : null,
              ...f
            }
