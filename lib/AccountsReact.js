...
class _AccountsReact {
  ...
  determineSignupFields () {
    let signupFields = this.config.fields.signUp
    let filteredFields;

    switch (this.config.passwordSignupFields) {
      case 'EMAIL_ONLY':
        filteredFields = signupFields.filter(field => field._id !== 'username')
        break;
      case 'USERNAME_ONLY':
        filteredFields = signupFields.filter(field => field._id !== 'email')
        break;
      case 'USERNAME_AND_OPTIONAL_EMAIL':
        this.config.fields.signUp.forEach(field => {
          if (field._id === 'email') {
    ...
    if (filteredFields) {
      this.config.fields.signUp = filteredFields
    }
  }
