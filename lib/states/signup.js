  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        <a onMouseDown={this.redirectToSignIn}>{texts.links.toSignIn}</a>
      </Fragment>
    )
  }
  ...
  onSubmit = (e) => {
    ...
    createUser(newUser, err => {
      if (err) {
        ...
        if (typeof err.reason !== 'string' && !err) {
      ...
      // Check if set to login after signup
      if (this.defaults.loginAfterSignup) {
