  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        {emailSent && <div>{forgotPwdSubmitSuccess}</div>}

        <a onMouseDown={this.redirectToSignIn}>{this.defaults.texts.links.toSignIn}</a>
      </Fragment>
    )
  }
  ...
  onSubmit = (e) => {
    ...
    forgotPassword({ email: this.state.email }, err => {
      if (err) {
        ...
        const { onSubmitHook } = AccountsReact.config

        this.setState({ errors: [], emailSent: true })
        onSubmitHook(this.state.currentState)
      }
    })
  }
